...
| `HasSkipToday` as a repo-level `CountDocuments` query rather than application-layer state | The scheduler has no persistent state between hourly ticks — the only reliable way to detect a prior skip is a DB read. A `CountDocuments` query scoped to `{user_id, config_id, decision_type, skip_reason, timestamp >= todayStart}` is fast (covered by existing index on `user_id` + `timestamp`), returns a definitive answer, and keeps idempotency logic in the infrastructure layer where it belongs. Fail-safe on repo error (suppress write) prevents duplicates even when the guard itself errors. |
